...
# Keep the model sparse in RAM. Only expand the requested book's row below.
similarity = load_npz("similarity.npz").tocsr()
...
def recommend(book_title, n=5):
    ...
    row = similarity.getrow(idx).toarray().ravel()
